...
//		inactivityTimer.shutdown();
    ...
    public void handleDecode(Result result, Bitmap barcode) {
        ...
        initZXingCamera();
        if(result != null){
            QrScanProxy.getInstance().getCallBack().OnReceiveDecodeResult(CaptureActivity.this, result.getText().toString());
            SharedPreferences sPreferences=this.getSharedPreferences("config",MODE_PRIVATE);
            SharedPreferences.Editor editor=sPreferences.edit();
            editor.putString("coderesult", result.getText().toString());
            editor.commit();
            finish();
        }else {
