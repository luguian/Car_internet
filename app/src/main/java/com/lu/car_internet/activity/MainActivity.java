...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		weizhangIntent.putExtra("appId",1831);// 你的appid
		weizhangIntent.putExtra("appKey", "9a322394d2d924fdfe5ec335a5281c00");// 你的appKey
		...
		btn_cpsz.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				Toast.makeText(MainActivity.this, short_name.getText(),
		...
		query_city.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if(!isNetworkAvailable(MainActivity.this)){
					Toast.makeText(MainActivity.this, "请链接移动网络WIFI",
							Toast.LENGTH_SHORT).show();
				}else{
					List<ProvinceInfoJson> provinceList = WeizhangClient.getAllProvince();
					if(provinceList == null){
						Toast.makeText(MainActivity.this, "由于本地没有缓存省份，正在从网上获取省份，请稍等", Toast.LENGTH_SHORT).show();
					}else{
					Intent intent = new Intent();
	...
	private boolean checkQueryItem(CarInfo car) {
		...
		if (car.getChepai_no().length() != 7) {
			Toast.makeText(MainActivity.this, "你输入的车牌号有误", Toast.LENGTH_SHORT).show();
			return false;
		}
